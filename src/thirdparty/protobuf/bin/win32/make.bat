...
@if not exist %CORE_DIR%\usermessages mkdir %CORE_DIR%\usermessages
@if not exist %CORE_DIR%\usermessages\%GAME% mkdir %CORE_DIR%\usermessages\%GAME%

@echo Running boost generation code on %GAME%\usermessages.proto...
@.\protoc.exe --proto_path=%PROTOBUF_SRC_DIR% --bplbindings_out=%CORE_DIR%\usermessages %PROTOFILE_DIR%\%GAME%\usermessages.proto

@echo Renaming file as the boost python exporter doesn't name them correctly
@xcopy %CORE_DIR%\usermessages\_%GAME%\usermessages.cpp %CORE_DIR%\usermessages\%GAME%\bindings.cpp /y /i /v
@rd %CORE_DIR%\usermessages\_%GAME% /S /Q
