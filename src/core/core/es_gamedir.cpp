...
{
	if( !engine ) {
		Msg("[ES] Could not initialize game paths! Engine pointer was invalid.\n");
		return false;
	}

	engine->GetGameDir(m_szGameDir, MAX_GAME_PATH);
	V_snprintf(m_szESDir, MAX_GAME_PATH, "%s%s", m_szGameDir, ES_ADDON_BASE);

	// Fix slashes
	V_FixSlashes(m_szGameDir);
	V_FixSlashes(m_szESDir);

	DevMsg(1, "[ES] Game directory is %s\n", m_szGameDir);
	DevMsg(1, "[ES] Eventscripts directory is %s\n", m_szESDir);
