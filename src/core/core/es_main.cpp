...
#include "es_python.h"
#include "es_main.h"
#include "es_gamedir.h"
#include "addons/es_addon.h"
#include "interface.h"
...
//---------------------------------------------------------------------------------
CEventscriptsPlugin g_EventscriptsPlugin;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEventscriptsPlugin, IServerPluginCallbacks, INTERFACEVERSION_ISERVERPLUGINCALLBACKS, g_EventscriptsPlugin );
...
bool GetInterfaces( InterfaceHelper_t* pInterfaceList, CreateInterfaceFn factory ) 
{
	...
	while( pInterface->szInterface )
	{
		...
		*pGlobal = factory(pInterface->szInterface, NULL);
		...
		if( *pGlobal ) {
			Msg("[ES] Interface %s at %x\n", pInterface->szInterface, *pGlobal);
		} else {
			Warning("[ES] Could not retrieve interface %s\n", pInterface->szInterface);
			return false;
		}
...
bool AddEventsFromFile( const char* szRelativePath )
{
	...
	DevMsg(1, "[ES] Begin event registration for %s.\n", szRelativePath);
	...
	DevMsg(1, "[ES] Event registration finished. A total of %d events were registered.\n", count);
	return true;
}
...
bool CEventscriptsPlugin::Load(	CreateInterfaceFn interfaceFactory, CreateInterfaceFn gameServerFactory )
{
	...
	if( !AddEventsFromFile("resource/modevents.res") ) {
		Msg("[ES] ERROR: Could not load modevents.res\n");
		return false;
	}

	if( !AddEventsFromFile("resource/gameevents.res") ) {
		Msg("[ES] ERROR: Could not load gameevents.res\n");
		return false;
	}

	if( !AddEventsFromFile("resource/serverevents.res") ) {
		Msg("[ES] ERROR: Could not load serverevents.res\n");
		return false;
	}
	...
	if( !AddEventsFromFile("resource/replayevents.res") ) {
		Msg("[ES] ERROR: Could not load replayevents.res\n");
		return false;
	}
...
//---------------------------------------------------------------------------------
void CEventscriptsPlugin::FireGameEvent( IGameEvent * event )
{
	const char * name = event->GetName();
	DevMsg(1, "CEventscriptsPlugin::FireGameEvent: Got event \"%s\"\n", name );
