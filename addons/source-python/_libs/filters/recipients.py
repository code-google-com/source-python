...
class RecipientFilter(CMRecipientFilter):
    '''Class used to improve the ease of use of a CMRecipientFilter
        instance'''
        
    def __init__(self, *filters):
        '''Initialize the recipient and update its content matching the given
            filters'''
    ...
    def __contain__(self, index):
        '''Return True if the given index is in the recipient, False
            otherwise'''
    ...
    def __iter__(self):
        ...
        # Return a mapping of the recipient filter
        return map(self.get_recipient_index, range(len(self)))
